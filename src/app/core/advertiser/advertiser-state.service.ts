...
import { withLatestFrom, map, mergeMap, share, publish, refCount } from 'rxjs/operators';
...
@Injectable()
export class AdvertiserStateService {
  ...
  findAdvertiserByUri(uri: string): Observable<Advertiser> | undefined {
    return this.advertisers.pipe(map((advertisers: Advertiser[]) => advertisers.find(advertiser => advertiser.set_advertiser_uri === uri)));
  }
  ...
  addAdvertiser(name: string): Observable<{ id: number; name: string; set_advertiser_uri: string }> {
    ...
    post.pipe(withLatestFrom(this._advertisers)).subscribe(([newAdvertiser, advertisers]) => {
